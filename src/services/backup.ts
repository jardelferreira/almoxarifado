...
function validarReferencias(tabelas: BackupTabelas, projeto: BackupProjetoIdentidade | null, erros: string[], avisos: string[]): void {
  ...
  for (const movimento of tabelas.movimentacoes_equipamentos ?? []) {
    ...
    for (const campo of ["origem_id", "destino_id"]) {
      const valor = item[campo];
      const parte = item[campo === "origem_id" ? "tipo_origem" : "tipo_destino"];
      if (!valor || !parte) continue;
...
export async function criarBackupDados(projetoId: string): Promise<BackupArquivo> {
  ...
  const tabelas = Object.fromEntries(
    BACKUP_TABELAS.map((nome) => {
      ...
      if (TABELAS_GLOBAIS.includes(nome)) return [nome, registros];
...
async function apagarEscopoProjeto(
  ...
  const idsEquipes = new Set((tabelas.equipes ?? []).map((item) => String((item as BackupRegistro).id)));
  const idsDocumentos = new Set((tabelas.documentos ?? []).map((item) => String((item as BackupRegistro).id)));
  const idsInventarios = new Set((tabelas.inventarios ?? []).map((item) => String((item as BackupRegistro).id)));
  const idsEstoques = new Set((tabelas.estoque_equipamentos ?? []).map((item) => String((item as BackupRegistro).id)));
